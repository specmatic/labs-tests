...
from lablib.scaffold import ArtifactSpec, ValidationContext, assert_condition, detail


def files(upstream_lab: Path) -> dict[str, Path]:
    return {
        "finance_11": upstream_lab / "examples" / "test_finance_user_11.json",
        "support_55": upstream_lab / "examples" / "test_support_user_55.json",
    }


def common_artifact_specs() -> tuple[ArtifactSpec, ...]:
    return (
        ArtifactSpec(
...
def phase_file_transforms(phase_id: str, phase_title: str) -> dict:
    if phase_id == "baseline":
        return {"finance_11": set_finance_baseline, "support_55": set_support_baseline}
    if phase_id == "intermediate":
        return {"finance_11": set_finance_task_a, "support_55": set_support_baseline}
    if phase_id == "final":
        return {"finance_11": set_finance_task_a, "support_55": set_support_final}
    return {}
...
def fix_summary(phase_id: str, phase_title: str) -> tuple[str, ...]:
    if phase_id == "intermediate":
        return ("Changed examples/test_finance_user_11.json so decision uses $match(pattern: approved|verified).",)
    if phase_id == "final":
        return (
